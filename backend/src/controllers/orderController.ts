            ...
            const order = await prisma.$transaction(async (tx) => {
                ...
                const newOrder = await tx.order.create({
                    data: {
                        ...
                        deliveryCity: data.deliveryType === 'pickup' ? null : (data.deliveryCity ?? null),
                        deliveryPvzCode: data.deliveryType === 'pickup' ? null : (data.deliveryPvzCode ?? null),
                        deliveryPvzAddress: data.deliveryType === 'pickup' ? null : (data.deliveryPvzAddress ?? null),
                        deliveryPrice: data.deliveryType === 'pickup' ? 0 : (data.deliveryPrice ?? null),
                        items: {
