...
export default function CartPage() {
  ...
  const grandTotal = deliveryType === 'pickup'
    ? itemsTotal
    : itemsTotal + (selectedPvz ? deliveryPrice : 0);
  ...
  async function handleSubmit() {
    ...
    if (!valid) return;
    if (deliveryType === 'delivery' && !selectedPvz) {
    ...
    try {
      ...
      const order = await createOrder({
        ...
        deliveryType,
        ...(deliveryType === 'delivery' && selectedPvz ? {
  ...
  return (
    <div className="container py-3" style={{ maxWidth: 600, paddingBottom: 80 }}>
      ...
      {step === 'delivery' && (
        <>
          <h5 className="fw-bold mb-3">Способ получения</h5>

          {/* ── Delivery type toggle ── */}
          <div className="d-flex gap-2 mb-4">
            <div
              className={`flex-fill text-center py-3 rounded border-2 border ${
            ...
            >
              ...
              <div className={`fw-semibold small mt-1 ${deliveryType === 'delivery' ? 'text-primary' : 'text-muted'}`}>
                Доставка СДЭК
              </div>
            </div>
            <div
            ...
            >
              <div style={{ fontSize: 24 }}>🏪</div>
              <div className={`fw-semibold small mt-1 ${deliveryType === 'pickup' ? 'text-primary' : 'text-muted'}`}>
                Самовывоз
              </div>
            </div>
          </div>
          ...
          {deliveryType === 'pickup' && (
            <>
              <div className="alert alert-success py-3">
                <div className="fw-semibold mb-1">🏪 Самовывоз</div>
                <div className="small text-muted">
                  Адрес и время самовывоза уточняйте у менеджера после оформления заказа.
          ...
          <button
            ...
            disabled={(deliveryType === 'delivery' && !selectedPvz) || submitting}
